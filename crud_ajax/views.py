...
class BusStopSelection(View):
    def post(self, request):
        ...
        for j in range(len(passengerDetails)):
            ...
            print('pickup details _________________________ ')
            print(passengerDetails[j]['pickup'])
            #loc = geolocator.geocode(passengerDetails[j]['pickup'])[0]
            lat, lng = passengerDetails[j]['pickup'].split(',')
            
            print(lat, lng)
            # print('..............................')
            # print(passengerDetails[j]['pickup'])
            # print(loc)
            X,Y,Z,D = utm.from_latlon(float(lat), float(lng))
            stud_data[j] = (X, Y)
        ...
        for j in range(len(potentialBusStops)):
            ...
            lat , lng = potentialBusStops[j].split(',')
            X,Y,Z,D = utm.from_latlon(float(lat), float(lng))
            stop_data[j] = (X, Y)
